...
import { extractionQuality, pdfOrigin } from "./extractionQuality";
import { aiResponse, hasAiResponse } from "./aiService";
import { scoreCVWithBreakdown, hasScore } from "./cvScoring";
import { canSpend, canAnonAnalyze, consumeAnonAnalyze } from "../../shared/billing/quotaService";
...
import { renderedCvAnalysisArtifact, savedCvAnalysisArtifact } from "../../shared/savedCvAnalysisService";
...
export const analyzeCVController = async (req: Request, res: Response) => {
  ...
  try {
      ...
      : savedCv
        ...
        : builderCv
          ? await renderedCvAnalysisArtifact({
              ...
              sectionGap: typeof builderCv.sectionGap === "number" ? builderCv.sectionGap : undefined,
            })
    ...
    logger.info("[cv-analyze] scoring summary", {
      qualityScore,
      impactDimension:
    ...
    try {
      ...
      if (quality.suspect) {
        logger.warn("[cv-analyze] extraction looks incomplete", { ...quality, ...origin });
      }
      ...
      logger.error("[cv-analyze] failed to record AnalysisEvent", e);
    }
    ...
    logger.error("CV analyze error:", error);
