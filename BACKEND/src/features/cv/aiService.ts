...
import { translateProseDetailed } from "../../shared/translateProseService";
...
export async function aiResponse(
  ...
  if (stored) logger.error("[ai-response] cached result is untranslated, regenerating");
  ...
  if (language !== "en") {
    ...
    if (!complete) {
      logger.error("[ai-response] partial translation, not caching");
      return result;
    }
