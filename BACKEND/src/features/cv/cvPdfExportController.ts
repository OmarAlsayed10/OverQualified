...
export const exportCvPdfController = async (
  ...
  try {
    const { formData, sectionOrder, template, fontScale, sectionGap, name } = req.body ?? {};
    if (!formData || typeof formData !== "object") {
      res.status(400).json({ message: "formData is required" });
      return;
    }

    const { pdf, pageCount } = await renderCvPdf({ formData, sectionOrder, template, fontScale, sectionGap });
