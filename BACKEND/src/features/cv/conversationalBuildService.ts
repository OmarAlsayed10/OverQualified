...
// The prompt tells the model to invent nothing, so a fact the user never gave comes back as
// null or missing. That is the correct answer, not a broken response: accept it as "" and let
// the builder's own field validation tell the user which required fields are still empty.
...
const text = z.unknown().transform(scalarText);

// The prompt asks for one "- " bullet per line, but the model often answers with a JSON array
// of bullets instead. Same content, different container — join it rather than reject the reply.
const bulletText = z.unknown().transform((value) => {
...
const list = <T extends z.ZodTypeAny>(item: T) =>
  z.array(item).nullish().transform((value) => value ?? []);
...
const conversationalBuildResponseSchema = z.object({
  ...
  formData: z.object({
    personalInfo: z.object({
      firstName: text, lastName: text, email: text, phoneCode: text,
      phone: text, country: text, city: text, town: text,
      professionalTitle: text, ProfessionalSummary: text, linkedin: text,
      github: text, portfolio: text,
    }),
    experience: list(z.object({
      jobTitle: text, company: text, location: text, startDate: text,
      endDate: text, description: bulletText,
    })),
    education: list(z.object({
      status: z.enum(["graduated", "undergraduate"]).nullish().transform((value) => value ?? "graduated"),
      institution: text, degree: text, location: text, startYear: text,
      endYear: text, description: bulletText,
    })),
    projects: list(z.object({
      name: text, technologies: text, demoUrl: text, githubUrl: text,
      description: bulletText,
    })),
    skills: z.object({
      skillCategories: list(z.object({
        name: text,
        skills: list(z.string()),
      })).optional(),
      skills: list(z.string()).optional(),
      languages: z.string().optional().default(""),
      certifications: list(z.object({
        name: text, issuer: text, date: text, url: text, description: bulletText,
      })).optional().default([]),
    }),
  }),
...
Every education entry carries a "status": "graduated" when the degree is finished, and "undergraduate" when the person is still studying (leave endYear empty unless they gave an expected graduation year). Never assume "graduated" from a start year alone.
