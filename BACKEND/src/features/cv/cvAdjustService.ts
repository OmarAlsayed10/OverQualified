...
FORMDATA_START
{
  ...
  "education": [{"status":"","institution":"","degree":"","location":"","startYear":"","endYear":"","description":""}],
...
- skills is a flat array of individual skills, never one comma-joined string.`;
  ...
  const parseFormData = (raw: string): BuilderFormData | null => {
    ...
    try {
      ...
      logger.warn("cvAdjustService: FORMDATA block was not usable JSON", error);
      return null;
    }
  };
  ...
  for (let attempt = 0; attempt < 3; attempt++) {
    ...
    if (!cvMatch) {
      logger.error("cvAdjustService: CV_START/CV_END markers missing in response:\n", raw.slice(0, 500));
    ...
    if (invented.length === 0) {
      logger.warn(`cvAdjustService: ${weakBullets.length} weak bullet opening(s) — retry ${attempt + 1}/2`);
    ...
    logger.warn(`cvAdjustService: rewrite invented metrics ${JSON.stringify(invented)} — retry ${attempt + 1}/2`);
    messages.push({ role: "assistant", content: raw });
    messages.push({
      role: "user",
  ...
  if (lastGoodRewrite) {
    logger.warn("cvAdjustService: weak bullet openings survived every retry — returning the rewrite anyway.");
    return lastGoodRewrite;
  }

  logger.error("cvAdjustService: could not optimize without fabricating metrics — returning original CV unchanged.");
  return {
    adjustedCV: cvText,
    formData: null,
    changes: [
