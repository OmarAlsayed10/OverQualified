...
import { createCV, getCVsByUser } from "../cvBuilderService";
...
export const saveCV = async (request: Request, response: Response, next: NextFunction) => {
  try {
    ...
    const { title, template, sectionOrder, customSections, fontScale, sectionGap, personalInfo, experience, education, projects, skills } = request.body;
