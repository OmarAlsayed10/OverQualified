...
7. CROSS-SECTION CONSOLIDATION: content repeated across experience and project descriptions should survive in ONE place only. If a skill is already evident from the descriptions, drop it from the summary.`;
  ...
  for (let attempt = 0; attempt < 2; attempt++) {
    ...
    logger.warn(`optimizeCvLength: rewrite still ${attemptWords} words (target ${targetWords}) — retrying`);
  ...
  try {
    ...
    if (parsedExpCount < expCount || parsedEduCount < eduCount || parsedProjCount < projCount) {
      logger.warn(`optimizeCvLength: LLM dropped entries (exp: ${expCount}->${parsedExpCount}, edu: ${eduCount}->${parsedEduCount}, proj: ${projCount}->${parsedProjCount}). Merging descriptions only.`);
...
// These two hold one bullet per line; every other section is a single run of text.
...
// The model writes "find‑nearest" with a non-breaking hyphen and "end—to—end" with dashes an ATS
// parser does not recognise, and pads lines with trailing spaces. Neither survives a round trip
// through the templates intact, so the text is flattened to ASCII before it reaches the field.
const cleanLine = (line: string): string => {
  const ascii = TYPOGRAPHY.reduce((text, [pattern, replacement]) => text.replace(pattern, replacement), line);
  return ascii.replace(/\*\*/g, "").replace(/^[-*•]\s*/, "").replace(/\s+/g, " ").trim();
};

const words = (text: string): string[] => text.toLowerCase().match(/[a-z0-9]+/g) ?? [];

// Asked to edit a description the model likes to re-print the entry's own header above the
// bullets ("Frontend Web Developer, Penta-B"), which then renders inside the bullet block in every
// template. A line built entirely from words already in the entry's header is that header, not a
// bullet — a real bullet always carries at least one word the header does not.
const isHeaderEcho = (line: string, headerWords: Set<string>): boolean => {
  const lineWords = words(line);
  return lineWords.length > 0 && lineWords.every((word) => headerWords.has(word));
};
...
export async function editFieldWithAI(
  ...
  const wantsBullets = BULLET_SECTIONS.has(sectionName.toLowerCase());
  const headerWords = new Set([
    ...words(sectionName),
    ...Object.values(context).flatMap((value) => words(String(value ?? ""))),
  ]);
  ...
  const outputContract = wantsBullets
    ? `Return JSON: {"bullets": ["first bullet", "second bullet"]}
- One achievement per array entry. No leading "-", "*" or "•" — the array IS the bullet list.
- Never put the job title, company, project name, or a section heading in the array. The entry already has those; the array holds achievements only.`
    : `Return JSON: {"text": "the finished content"}
- One string. No heading, no label, no bullet markers, no line breaks unless the content genuinely needs them.`;

  const response = await groqChat({
    model: MODELS.versatile,
    response_format: { type: "json_object" },
    // gpt-oss spends completion tokens on reasoning before it writes anything. At the previous
    // budget a long description burned the whole allowance thinking and returned an empty string
    // or a bullet cut mid-word. This work is rewriting, not reasoning, so the effort goes low and
    // the budget goes up — which also takes ~10s of latency off a single field edit.
    reasoning_effort: "low",
    messages: [
      {
        role: "system",
        content: `You are a professional CV writing assistant. You edit and generate CV content based on the user's instructions. NEVER ask follow-up questions — just return the content.
...
- No graphics, icons, tables, or special characters. Use plain ASCII hyphens and quotes, never typographic ones.
- Keep descriptions to 3-4 bullets per entry, unless the user asks for a specific number — then give exactly that many
...
${cvContext ? `\nCANDIDATE'S FULL CV CONTEXT:\n${cvContext}\n\nYou have the candidate's full CV context. Use it to: avoid repeating action verbs or achievements already used in other entries, reference relevant skills/projects/education when writing experience bullets, and ensure tone and style consistency across sections. Check other entries to ensure no verb duplication and complementary achievement framing.` : ""}`
  ...
  try {
    ...
    // A malformed JSON body still usually contains the prose the user asked for, and losing the
    // edit is worse than losing the structure.
    payload = wantsBullets ? { bullets: raw.split("\n") } : { text: raw };
  }
  ...
  const bullets = (Array.isArray(payload?.bullets) ? payload.bullets : String(payload?.text ?? raw).split("\n"))
    .map((bullet: unknown) => cleanLine(String(bullet ?? "")))
    .filter(Boolean);
  const kept = bullets.filter((bullet: string) => !isHeaderEcho(bullet, headerWords));
  // A one-line entry whose only line reads as the header is still the user's content; dropping it
  // would hand back an empty field.
  return (kept.length > 0 ? kept : bullets).join("\n");
}
