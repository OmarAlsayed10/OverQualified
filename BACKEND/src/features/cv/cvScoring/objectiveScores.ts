...
export function atsCompatibilityObjective(
  ...
  if (SHOULD_DEBUG_CV_SCORING) {
    logger.info("[cv-score] ats compatibility debug", { score, ...checks });
  }
...
export function formattingLayoutObjective(
  ...
  if (SHOULD_DEBUG_CV_SCORING) {
    logger.info("[cv-score] formatting layout debug", { score, ...checks });
  }
...
export function experienceObjective(
  ...
  if (SHOULD_DEBUG_CV_SCORING) {
    logger.info("[cv-score] impact scoring debug", {
