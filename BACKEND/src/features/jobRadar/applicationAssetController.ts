...
import { generateVariants } from "./cvVariantService";
...
export const generateVariantsController = async (request: Request, response: Response): Promise<void> => {
  ...
  const variants = await prisma.$transaction(async (transaction) => {
    await transaction.cVVariant.deleteMany({ where: { userId, jobMatchId: id } });
    await transaction.jobMatch.update({ where: { id }, data: { selectedCvVariant: null } });
    return Promise.all(generatedVariants.map((variant) => transaction.cVVariant.create({
