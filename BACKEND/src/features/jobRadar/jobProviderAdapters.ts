...
import { Preference, RawJob, tokenize } from "../../shared/jobs/jobMatchScoring";
import { JobMarket, PRIMARY_MARKET } from "../../shared/jobs/jobMarkets";

const job = (source: string, values: Omit<RawJob, "source">): RawJob => ({ source, ...values });

const JSEARCH_PAGES = Math.max(1, Number(process.env.JSEARCH_PAGES ?? 1));
...
export async function fetchJooble(preference: Preference, market: JobMarket = PRIMARY_MARKET): Promise<RawJob[]> {
  const key = process.env.JOOBLE_KEY;
  if (!key) return [];
  const { data } = await axios.post(`https://jooble.org/api/${key}`, { keywords: `${preference.role} ${preference.keywords ?? ""}`.trim(), location: preference.location || process.env.JOOBLE_LOCATION || market.label }, { timeout: 10000 });
  return (data.jobs || []).map((sourceJob: any) => job("jooble", {
    externalId: String(sourceJob.id ?? sourceJob.link), title: sourceJob.title || "", company: sourceJob.company || "Unknown", location: sourceJob.location || null, country: market.country, url: sourceJob.link || "", postedAt: sourceJob.updated ? new Date(sourceJob.updated) : null, description: normalizeJobDescription(sourceJob.snippet || "").plainText,
  }));
}
...
export async function fetchJSearch(preference: Preference, market: JobMarket = PRIMARY_MARKET): Promise<RawJob[]> {
  const key = process.env.JSEARCH_KEY;
  if (!key) return [];
  const query = `${preference.role} ${preference.keywords ?? ""} in ${preference.location || market.label}`.trim();
  const { data } = await axios.get("https://jsearch.p.rapidapi.com/search", { params: { query, page: 1, num_pages: JSEARCH_PAGES, country: market.jsearchCountry, date_posted: "month" }, headers: { "X-RapidAPI-Key": key, "X-RapidAPI-Host": "jsearch.p.rapidapi.com" }, timeout: 15000 });
  return (data.data || []).map((sourceJob: any) => job("jsearch", {
    externalId: String(sourceJob.job_id), title: sourceJob.job_title || "", company: sourceJob.employer_name || "Unknown", location: [sourceJob.job_city, sourceJob.job_country].filter(Boolean).join(", ") || null, country: market.country, url: sourceJob.job_apply_link || sourceJob.job_google_link || "", postedAt: sourceJob.job_posted_at_datetime_utc ? new Date(sourceJob.job_posted_at_datetime_utc) : null, description: normalizeJobDescription(sourceJob.job_description || "").plainText,
  }));
}

export async function fetchCareerjet(preference: Preference, market: JobMarket = PRIMARY_MARKET): Promise<RawJob[]> {
  const affid = process.env.CAREERJET_AFFID;
  if (!affid) return [];
  const { data } = await axios.get("http://public.api.careerjet.net/search", { params: { affid, keywords: `${preference.role} ${preference.keywords ?? ""}`.trim(), location: preference.location || market.label, locale_code: market.careerjetLocale, pagesize: 25, contenttype: "application/json", sort: "date" }, timeout: 10000 });
  return (data.jobs || []).map((sourceJob: any) => job("careerjet", {
    externalId: String(sourceJob.url), title: sourceJob.title || "", company: sourceJob.company || "Unknown", location: sourceJob.locations || null, country: market.country, url: sourceJob.url || "", postedAt: sourceJob.date ? new Date(sourceJob.date) : null, description: normalizeJobDescription(sourceJob.description || "").plainText,
  }));
}
