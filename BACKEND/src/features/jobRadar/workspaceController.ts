...
import { cvToPlainText } from "../../shared/documents/documentService";
import { generateScreeningQuestions } from "./screeningQuestionService";
...
export const updateWorkspaceController = async (request: Request, response: Response): Promise<void> => {
  const userId = authenticatedUserId(request);
  const { id } = request.params;
  const { checklist, screeningAnswers, notes, reminderAt, selectedCvVariant, status, coverLetter, workspaceData } = request.body;
  const match = await prisma.jobMatch.findFirst({ where: { id, userId } });
  if (!match) {
  ...
  if (workspaceData !== undefined) {
    if (typeof workspaceData !== "object" || workspaceData === null || Array.isArray(workspaceData)) {
      response.status(400).json({ message: "Workspace data is invalid." });
      return;
    }
    updateFields.workspaceData = workspaceData;
  }
...
export const generateScreeningAnswersController = async (request: Request, response: Response): Promise<void> => {
  ...
  const suppliedCvText = typeof request.body.cvText === "string" ? request.body.cvText.trim().slice(0, 30000) : "";
  const primaryCv = suppliedCvText ? null : await getPrimaryCV(userId);
  const cvText = suppliedCvText || (primaryCv ? cvToPlainText(primaryCv) : "");
  if (!cvText) {
    response.status(400).json({ message: "No CV found." });
    return;
  }
