...
export async function getOrGenerateSkillRoadmap(skillName: string, category: string = "skill"): Promise<SkillRoadmapData> {
  ...
  try {
    ...
    logger.warn("DB lookup for skill roadmap failed, falling back:", err);
  }
  ...
  if (STATIC_ROADMAPS[staticKey]) {
    ...
    try {
      ...
      logger.warn("Failed to persist static roadmap to DB:", err);
    }
    return staticData;
  }
...
Return JSON only in this exact format:
{
  ...
  try {
    ...
    try {
      ...
      logger.warn("Failed to cache AI skill roadmap to DB:", dbErr);
    }

    return result;
  } catch (aiErr) {
    logger.error("AI Skill Roadmap generation failed, using fallback:", aiErr);
