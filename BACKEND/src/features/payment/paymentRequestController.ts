...
import { submitCustomPaymentRequest } from "./creditPurchaseService";
import { PricingConfigurationError } from "./creditPricingService";
...
export const submitPaymentController = async (request: Request, response: Response): Promise<void> => {
  ...
  try {
    ...
    try {
      ...
      logger.error("Admin notification email failed (payment still recorded):", emailError);
    }
