...
export async function listInterviewSessions(userId: string) {
  ...
  return documents.flatMap((document) => {
    ...
    // Interviews quit before they were deleted on quit stay in the database; they are just
    // as unresumable as the ones deleted now, so they are left out of the history.
    if (!session || session.status === "quit") return [];
    return [publicSession(document, session)];
  });
}
...
// A quit interview cannot be resumed and carries no report, so it is deleted rather than
// kept as a dead row in the user's history. Only finished interviews are worth keeping.
export async function quitInterviewSession(userId: string, id: string) {
  ...
  const { count } = await prisma.document.deleteMany({ where: { id, userId, type: SESSION_TYPE } });
  if (count === 0) return { kind: "not_found" as const };
  return { kind: "success" as const, id };
}
